    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var renderAction in renderActions)
            {
                ...
                RenderPreview(zplData, methodName);
            }
        }
        ...
        static void RenderPreview(string zplData, string methodName)
        {
            var zplFilename = string.Format("{0}-{1}.txt", methodName, Guid.NewGuid());
            File.WriteAllText(zplFilename, zplData);
            ...
            using (var client = new LabelaryClient())
            {
                ...
                var fileName = string.Format("{0}-{1}.png", methodName, Guid.NewGuid());
